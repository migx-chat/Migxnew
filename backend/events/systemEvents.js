    ...
    try {
      ...
      if (userId && username) {
        ...
        for (const roomId of activeRooms) {
          ...
          // Only send "has left" message if not already sent by logout handler
          if (!socket.leftMessageSent) {
            const roomService = require('../services/roomService');
            const { getUserLevel } = require('../services/userLevelService');
            ...
            io.to(`room:${roomId}`).emit('chat:message', {
              ...
              message: `${username} [${userLevel}] has left`,
              type: 'presence',
