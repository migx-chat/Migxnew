...
const startGame = async (roomId, userId, username, amount) => {
  ...
  try {
    ...
    // Insert game to database and get the actual database ID
    let dbGameId = gameId;
    try {
      ...
      if (insertResult.rows && insertResult.rows[0]) {
        ...
        game.id = dbGameId; // Update game.id with actual database ID
        game.dbId = dbGameId; // Also store as dbId for clarity
      }
    } catch (err) {
...
const tallyRound = async (roomId, isTimedOut = false) => {
  ...
  if (remainingPlayers.length === 1) {
    ...
    // Use game.id (database ID) for updates
    if (game.id) {
      await query(
        `UPDATE lowcard_games SET status = 'finished', winner_id = $1, winner_username = $2, pot_amount = $3, finished_at = NOW()
         WHERE id = $4`,
      ...
      logger.error('LOWCARD_MISSING_GAME_ID', { roomId, pot: game.pot });
    }
